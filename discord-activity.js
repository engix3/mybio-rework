...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function updateStatus() {
    try {
      ...
      // Обновляем иконку активности
      if (data.activity.type === 'game') {
      ...
      } else if (data.activity.type === 'music') {
      ...
      } else if (data.activity.type === 'stream') {
      ...
      let text = data.activity.name;
      if (data.activity.details) text += `: ${data.activity.details}`;
      if (data.activity.state) text += ` - ${data.activity.state}`;
